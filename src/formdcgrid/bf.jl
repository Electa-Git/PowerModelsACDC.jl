...
```
"""
function constraint_ohms_dc_branch(pm::AbstractBFQPModel, n::Int, f_bus, t_bus, f_idx, t_idx, r, p)
    l = f_idx[1];
    p_dc_fr = PowerModels.var(pm, n,  :p_dcgrid, f_idx)
    p_dc_to = PowerModels.var(pm, n,  :p_dcgrid, t_idx)
    ccm_dcgrid = PowerModels.var(pm, n,  :ccm_dcgrid, l)
...
function constraint_ohms_dc_branch(pm::AbstractBFConicModel, n::Int, f_bus, t_bus, f_idx, t_idx, r, p)
    l = f_idx[1];
    p_dc_fr = PowerModels.var(pm, n,  :p_dcgrid, f_idx)
    p_dc_to = PowerModels.var(pm, n,  :p_dcgrid, t_idx)
    ccm_dcgrid = PowerModels.var(pm, n,  :ccm_dcgrid, l)
...
function constraint_dc_branch_current(pm::AbstractBFModel, n::Int, f_bus, f_idx, ccm_max, p)
# do nothing
end
