...
```
"""
function constraint_ohms_dc_branch(pm::GenericPowerModel{T}, n::Int, cnd::Int, f_bus, t_bus, f_idx, t_idx, r, p) where {T <: PowerModels.AbstractBFForm}
    l = f_idx[1];
    ...
    @constraint(pm.model, wdc_to == wdc_fr - 2 * (r/2) * p_dc_fr + (r)^2 * ccm_dcgrid)
end
