...
```
"""
function constraint_converter_losses(pm::AbstractDCPModel, n::Int,  i::Int, a, b, c, plmax)
    ...
    if pm.setting["conv_losses_mp"] == true
        PowerModels.con(pm, n, :conv_loss)[i] = @constraint(pm.model, pconv_ac + pconv_dc == a + b*cm_conv_ac )
    else
        PowerModels.con(pm, n, :conv_loss)[i] = @constraint(pm.model, pconv_ac + pconv_dc >= a + b*cm_conv_ac )
        PowerModels.con(pm, n, :conv_loss_aux)[i] = @constraint(pm.model, pconv_ac + pconv_dc >= a - b*cm_conv_ac )
        PowerModels.con(pm, n, :conv_loss_plmax)[i] = @constraint(pm.model, pconv_ac + pconv_dc <= plmax)
    end
end
"""
...
```
"""
function constraint_conv_transformer(pm::AbstractDCPModel, n::Int,  i::Int, rtf, xtf, acbus, tm, transformer)
    ...
    if transformer
        ...
        PowerModels.con(pm, n, :conv_tf_p_fr)[i] = @constraint(pm.model, ptf_fr == -btf*(v^2)/tm*(va-vaf))
        PowerModels.con(pm, n, :conv_tf_p_to)[i] = @constraint(pm.model, ptf_to == -btf*(v^2)/tm*(vaf-va))
    else
        PowerModels.con(pm, n, :conv_tf_p_fr)[i] = @constraint(pm.model, va == vaf)
        PowerModels.con(pm, n, :conv_tf_p_to)[i] = @constraint(pm.model, ptf_fr + ptf_to  == 0)
    end
end
"""
...
```
"""
function constraint_conv_reactor(pm::AbstractDCPModel, n::Int,  i::Int, rc, xc, reactor)
    ...
    if reactor
        bc = imag(1/(im*xc))
        v = 1 # pu, assumption DC approximation
        PowerModels.con(pm, n, :conv_pr_p)[i] = @constraint(pm.model, ppr_fr == -bc*(v^2)*(vaf-vac))
        PowerModels.con(pm, n, :conv_pr_p_to)[i] = @constraint(pm.model, pconv_ac == -bc*(v^2)*(vac-vaf))
    else
        PowerModels.con(pm, n, :conv_pr_p)[i] =  @constraint(pm.model, vac == vaf)
        PowerModels.con(pm, n, :conv_pr_p_to)[i] = @constraint(pm.model, ppr_fr + pconv_ac  == 0)
    end
end
"""
...
```
"""
function constraint_conv_filter(pm::AbstractDCPModel, n::Int,  i::Int, bv, filter)
    ppr_fr = PowerModels.var(pm, n, :pconv_pr_fr, i)
    ptf_to = PowerModels.var(pm, n, :pconv_tf_to, i)
...
```
"""
function constraint_converter_current(pm::AbstractDCPModel, n::Int,  i::Int, Umax, Imax)
    # not used
end
...
```
"""
function constraint_reactive_conv_setpoint(pm::AbstractDCPModel, n::Int,  i, qconv)
end
"""
...
```
"""
function constraint_conv_firing_angle(pm::AbstractDCPModel, n::Int,  i::Int, S, P1, Q1, P2, Q2)
end
"""
...
```
"""
function constraint_dc_droop_control(pm::AbstractDCPModel, n::Int,  i::Int, busdc_i, vref_dc, pref_dc, k_droop)
end
