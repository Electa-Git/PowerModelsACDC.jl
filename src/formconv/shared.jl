...
```
"""
function constraint_converter_losses(pm::AbstractWModels, n::Int, i::Int, a, b, c, plmax)
    ...
    PowerModels.con(pm, n,  :conv_loss)[i] = @constraint(pm.model, pconv_ac + pconv_dc == a + b*iconv + c*iconv_sq)
end
"""
...
```
"""
function constraint_conv_transformer(pm::AbstractWRModels, n::Int, i::Int, rtf, xtf, acbus, tm, transformer)
    ...
    w = PowerModels.var(pm, n,  :w, acbus)  # vm^2
    wf = PowerModels.var(pm, n,  :wf_ac, i)   # vmf * vmf
    wrf = PowerModels.var(pm, n,  :wrf_ac, i) # vm*vmf*cos(va-vaf) =  vmf*vm*cos(vaf-va)
    wif = PowerModels.var(pm, n,  :wif_ac, i) # vm*vmf*sin(va-vaf) = -vmf*vm*sin(vaf-va)
...
```
"""
function constraint_conv_reactor(pm::AbstractWRModels, n::Int, i::Int, rc, xc, reactor)
    ...
    wrc = PowerModels.var(pm, n,  :wrc_ac, i)
    wic = PowerModels.var(pm, n,  :wic_ac, i)
    ...
    if reactor
        ...
        PowerModels.con(pm, n,  :conv_pr_p)[i] = c1
        PowerModels.con(pm, n,  :conv_pr_q)[i] = c2
    else
        pcon, qcon = constraint_lossless_section(pm, wf, wc, wrc, wic, ppr_fr, ppr_to, qpr_fr, qpr_to)
        PowerModels.con(pm, n,  :conv_pr_p)[i] = pcon
        PowerModels.con(pm, n,  :conv_pr_q)[i] = qcon
    end
end
"""
...
```
"""
function constraint_conv_filter(pm::AbstractWModels, n::Int, i::Int, bv, filter)
    ppr_fr = PowerModels.var(pm, n,  :pconv_pr_fr, i)
    qpr_fr = PowerModels.var(pm, n,  :qconv_pr_fr, i)
    ptf_to = PowerModels.var(pm, n,  :pconv_tf_to, i)
    qtf_to = PowerModels.var(pm, n,  :qconv_tf_to, i)
    ...
    PowerModels.con(pm, n,  :conv_kcl_q)[i] = @constraint(pm.model, qpr_fr + qtf_to + -bv*filter*wf == 0)
end
...
```
"""
function constraint_conv_firing_angle(pm::AbstractWModels, n::Int, i::Int, S, P1, Q1, P2, Q2)
    pc = PowerModels.var(pm, n,  :pconv_ac, i)
    qc = PowerModels.var(pm, n,  :qconv_ac, i)
    coeff = (Q2-Q1)/(P2-P1)

    PowerModels.con(pm, n,  :conv_socphi)[i] = @constraint(pm.model, qc >= Q1 + (pc-P1) * coeff )
end
