...
```
"""
function constraint_converter_losses(pm::AbstractACPModel, n::Int, i::Int, a, b, c, plmax)
    pconv_ac = PowerModels.var(pm, n, :pconv_ac, i)
    pconv_dc = PowerModels.var(pm, n, :pconv_dc, i)
    iconv = PowerModels.var(pm, n, :iconv_ac, i)

    PowerModels.con(pm, n, :conv_loss)[i] = JuMP.@NLconstraint(pm.model, pconv_ac + pconv_dc == a + b*iconv + c*iconv^2)
end
"""
...
```
"""
function constraint_converter_current(pm::AbstractACPModel, n::Int, i::Int, Umax, Imax)
    vmc = PowerModels.var(pm, n, :vmc, i)
    pconv_ac = PowerModels.var(pm, n, :pconv_ac, i)
    qconv_ac = PowerModels.var(pm, n, :qconv_ac, i)
    iconv = PowerModels.var(pm, n, :iconv_ac, i)

    PowerModels.con(pm, n, :conv_i)[i] = @NLconstraint(pm.model, pconv_ac^2 + qconv_ac^2 == vmc^2 * iconv^2)
end
"""
...
```
"""
function constraint_conv_transformer(pm::AbstractACPModel, n::Int, i::Int, rtf, xtf, acbus, tm, transformer)
    ...
    vm = PowerModels.var(pm, n, :vm, acbus)
    va = PowerModels.var(pm, n, :va, acbus)
    vmf = PowerModels.var(pm, n, :vmf, i)
...
```
"""
function constraint_conv_reactor(pm::AbstractACPModel, n::Int, i::Int, rc, xc, reactor)
    pconv_ac = PowerModels.var(pm, n, :pconv_ac, i)
    qconv_ac = PowerModels.var(pm, n, :qconv_ac, i)
    ...
    if reactor
        ...
        gc = real(yc)
        bc = imag(yc)
        PowerModels.con(pm, n, :conv_pr_p)[i] = @NLconstraint(pm.model, -pconv_ac == gc*vmc^2 + -gc*vmc*vmf*cos(vac-vaf) + -bc*vmc*vmf*sin(vac-vaf))
        PowerModels.con(pm, n, :conv_pr_q)[i] = @NLconstraint(pm.model, -qconv_ac ==-bc*vmc^2 +  bc*vmc*vmf*cos(vac-vaf) + -gc*vmc*vmf*sin(vac-vaf))
...
```
"""
function constraint_conv_filter(pm::AbstractACPModel, n::Int, i::Int, bv, filter)
    ppr_fr = PowerModels.var(pm, n, :pconv_pr_fr, i)
    qpr_fr = PowerModels.var(pm, n, :qconv_pr_fr, i)
    ptf_to = PowerModels.var(pm, n, :pconv_tf_to, i)
    qtf_to = PowerModels.var(pm, n, :qconv_tf_to, i)
    ...
    PowerModels.con(pm, n, :conv_kcl_q)[i] = @NLconstraint(pm.model, qpr_fr + qtf_to +  (-bv) * filter *vmf^2 == 0)
end
"""
...
```
"""
function constraint_conv_firing_angle(pm::AbstractACPModel, n::Int, i::Int, S, P1, Q1, P2, Q2)
    p = PowerModels.var(pm, n, :pconv_ac, i)
    q = PowerModels.var(pm, n, :qconv_ac, i)
    phi = PowerModels.var(pm, n, :phiconv, i)

    PowerModels.con(pm, n, :conv_cosphi)[i] = @NLconstraint(pm.model,   p == cos(phi) * S)
    PowerModels.con(pm, n, :conv_sinphi)[i] = @NLconstraint(pm.model,   q == sin(phi) * S)
end

function constraint_dc_droop_control(pm::AbstractACPModel, n::Int, i::Int, busdc_i, vref_dc, pref_dc, k_droop)
    pconv_dc = PowerModels.var(pm, n, :pconv_dc, i)
    vdc = PowerModels.var(pm, n, :vdcm, busdc_i)

    PowerModels.con(pm, n, :conv_dc_droop)[i] = @constraint(pm.model, pconv_dc == pref_dc - sign(pref_dc) * 1 / k_droop * (vdc - vref_dc))
end
