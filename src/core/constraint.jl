...
function constraint_voltage_dc(pm::GenericPowerModel,  n::Int, cnd::Int)
end
...
```
"""
function constraint_kcl_shunt_dcgrid(pm::GenericPowerModel, n::Int, cnd::Int, i::Int, bus_arcs_dcgrid, bus_convs_dc, pd)
...
"`pconv[i] == pconv`"
function constraint_active_conv_setpoint(pm::GenericPowerModel, n::Int, cnd::Int, i, pconv)
    pconv_var = PowerModels.var(pm, n, cnd, :pconv_tf_fr, i)
    PowerModels.con(pm, n, cnd, :conv_pac)[i] = @constraint(pm.model, pconv_var == -pconv)
end

"`qconv[i] == qconv`"
function constraint_reactive_conv_setpoint(pm::GenericPowerModel, n::Int, cnd::Int, i, qconv)
