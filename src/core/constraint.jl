...
```
"""
function constraint_ohms_dc_branch(pm::GenericPowerModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, p)
    ...
    @constraint(pm.model, p_dc_fr == p * g * vmdc_fr * (vmdc_fr - vmdc_to))
    @constraint(pm.model, p_dc_to == p * g * vmdc_to * (vmdc_to - vmdc_fr))
end
...
```
"""
function constraint_converter_losses(pm::GenericPowerModel, n::Int, i::Int, a, b, c)
