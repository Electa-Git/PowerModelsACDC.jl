...
function run_mp_tnepopf_bf(data::Dict{String,Any}, model_type::Type{T}, solver; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!], setting = s, kwargs...) where T <: _PM.AbstractBFModel
    ...
    if haskey(data, "nw") && haskey(data["nw"]["1"], "ne_branch") # combined AC and DC TNEP
        return _PM.solve_model(data, model_type, solver, build_mp_acdctnepopf_bf; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    else
        return _PM.solve_model(data, model_type, solver, build_mp_tnepopf_bf; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!], setting = s, kwargs...)
    end
end

""
function build_mp_tnepopf_bf(pm::_PM.AbstractPowerModel)
...
""
function build_mp_acdctnepopf_bf(pm::_PM.AbstractPowerModel)
    for (n, networks) in pm.ref[:it][:pm][:nw]
        _PM.variable_bus_voltage(pm; nw = n)
        _PM.variable_gen_power(pm; nw = n)
        _PM.variable_branch_power(pm; nw = n)
