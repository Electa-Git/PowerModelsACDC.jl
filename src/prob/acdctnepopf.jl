export run_acdctnepopf

""
function run_acdctnepopf(file::String, model_type::Type, solver; kwargs...)
...
function run_acdctnepopf(data::Dict{String,Any}, model_type::Type, solver; kwargs...)
    return _PM.run_model(data, model_type, solver, post_acdctnepopf; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], kwargs...)
end
...
function ref_add_ne_branch!(ref::Dict{Symbol,<:Any}, data::Dict{String,<:Any})
    for (nw, nw_ref) in ref[:nw]
        ...
        if !haskey(nw_ref, :ne_buspairs)
            ...
            nw_ref[:ne_buspairs] = _PM.calc_buspair_parameters(nw_ref[:bus], nw_ref[:ne_branch], cid, ismc)
        end
    end
end
