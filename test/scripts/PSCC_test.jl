...
ipopt = optimizer_with_attributes(Ipopt.Optimizer, "tol" => 1e-4, "print_level" => 1)
juniper = optimizer_with_attributes(Juniper.Optimizer, "mip_solver" => cbc, "nl_solver" => ipopt)
...
##
resultDC = solve_tnep(data, PowerModels.DCPPowerModel, gurobi, setting=s)
resultDC["built_cv"], resultDC["built_br"] = display_results(data_pf1, resultDC)

resultSOCBF = solve_tnep(data, PowerModels.SOCBFPowerModel, gurobi, setting=s)
resultSOCBF["built_cv"], resultSOCBF["built_br"] = display_results(data_pf1, resultSOCBF)

resultSOCWR = solve_tnep(data, PowerModels.SOCWRPowerModel, gurobi, setting=s)
resultSOCWR["built_cv"], resultSOCWR["built_br"] = display_results(data_pf1, resultSOCWR)

resultQC = solve_tnep(data, PowerModels.QCRMPowerModel, gurobi; setting=s)
resultQC["built_cv"], resultQC["built_br"] = display_results(data_pf1, resultQC)

resultAC_juni = solve_tnep(data, PowerModels.ACPPowerModel, juniper; setting=s)
resultAC_juni["built_cv"], resultAC_juni["built_br"] = display_results(data_pf1, resultAC_juni)

resultLPAC = solve_tnep(data, PowerModels.LPACCPowerModel, gurobi; setting=s)
