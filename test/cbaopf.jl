@testset "CBA OPF" begin
    ...
    resultOPF = solve_acdcopf(data, PowerModels.DCPPowerModel, highs; setting=s)
    ...
    @testset "Redispatch OPF" begin
        ...
        dataRD = prepare_redispatch_opf_data(resultOPF["solution"], data; contingency=contingency, rd_cost_factor=rd_cost_factor)
        @testset "Redispatch OPF no control" begin
            ...
            resultRD_no_control = solve_rdopf(dataRD, PowerModels.DCPPowerModel, highs; setting=s)
        ...
        @testset "Redispatch OPF with control" begin
            ...
            resultRD_with_control = solve_rdopf(dataRD, PowerModels.DCPPowerModel, highs; setting=s)
            @test isapprox(resultRD_with_control["objective"], 19844.6, atol=1)
        end
    end

    @testset "SSSC model" begin
        data_sssc = PowerModels.parse_file(pkgdir(PowerModelsACDC, "test", "data", "case5_sssc.m"))
        process_additional_data!(data_sssc)
        ...
        @testset "DCP model" begin
            resultOPF = solve_acdcopf(data_sssc, PowerModels.DCPPowerModel, highs; setting=s)
